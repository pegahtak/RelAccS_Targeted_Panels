...
sample.names.2<- rownames(set3)
feature.names.2<- colnames(set3)

cancerTypes.2=unlist( map(strsplit(sample.names.2 , split = "_") ,1))
cancer_counts <- set3[ which(cancerTypes.2==Cancer), ]
if(Cancer=="KIDNEY")
  cancer_counts<- set3[ c(which(cancerTypes.2=="KIRC") , which(cancerTypes.2=="KIRP")), ]
if(Cancer=="LUNG")
  cancer_counts<- set3[ c(which(cancerTypes.2=="LUAD") , which(cancerTypes.2=="LUSC")), ]
...
PBMCsubject<- list.files(path="subjects/" , pattern = NULL)
PBMC.data<- data.frame(matrix(ncol=2 ,nrow=0))
colnames(PBMC.data)<- c("ID" , "bam")
for( k in 1:length(PBMCsubject))
{
  s<- as.vector(t(read.table(paste("subjects/" , PBMCsubject[k] , sep="") , header = FALSE)))
  ID<- c()
  bam<- c()
  for( l in 1:length(s))
  {
...
write.csv(f3_coor , file=paste(Cancer , "/f3_features.csv" , sep="")
          , append = FALSE )
### SampleID , Condition , Replicate , Counts

# {SampleID<- colnames(countMatrix)
# SampleID[ 1:nrow(cancer_counts)]<- paste(Cancer , "_" , seq(from=1 , nrow(cancer_counts) , by=1) , sep="") 
# Condition<- c(rep("Cancer" , nrow(cancer_counts) ) , rep("PBMC" , ncol(PBMC_counts) ) , rep("Neut" , ncol(Neutrophil_counts)))
# Counts<-  paste("DiffBind_counts/" ,colnames(countMatrix)  , sep="")
# Peaks<- rep("peaks.bed" , length(sampleID))
# sampleSheet<- data.frame(SampleID , Condition , Counts )
# write.csv(sampleSheet , "sampleSheet.csv" , quote = FALSE , row.names = FALSE)
# 
